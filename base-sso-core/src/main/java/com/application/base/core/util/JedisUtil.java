 ...
 * @Author: 孤狼
 * @desc: Redis 的管理工具.
 * https://www.cnblogs.com/NiceCui/archive/2017/12/20/8075103.html
 */
public class JedisUtil {
     ...
     */
    private static String hostInfos;
     ...
     * 主机密码.
     */
    private static String hostPasss;
    
    /**
     * 初始化.
     * @param address
     */
    public static void init(String address,String split,String passWords) {
        if (address==null || address.isEmpty()){
            throw new RuntimeException("not host and port info input.");
        }
    ...
    private static ShardedJedis getInstance() {
                ...
                if (INSTANCE_INIT_LOCL.tryLock(2, TimeUnit.SECONDS)) {
                    try {
                        if (shardedJedisPool == null) {
                            ...
                            if (org.apache.commons.lang3.StringUtils.isNotBlank(hostPasss)) {
                                isAuth=true;
                                authPassWord=hostPasss.split(passSplit);
                            }
                            ...
                            for (int i = 0; i <ipAndPorts.length ; i++) {
                                ...
                                if (authPassWord!=null && authPassWord.length>0){
                                    ...
                                    if (isAuth && org.apache.commons.lang3.StringUtils.isNotBlank(tmpAuth)){
                                        instance.setPassword(tmpAuth);
                                    }
                                }
                                jedisShardInfos.add(instance);
                            }
                            shardedJedisPool = new ShardedJedisPool(config, jedisShardInfos);
                            logger.info(">>>>>>>>>>> base-sso, JedisUtil.ShardedJedisPool init success.");
                        }
        ...
        if (shardedJedisPool == null) {
            throw new NullPointerException(">>>>>>>>>>> base-sso, JedisUtil.ShardedJedisPool is null.");
        }
